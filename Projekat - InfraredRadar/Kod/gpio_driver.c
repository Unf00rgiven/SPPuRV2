...
static int senzor0 = 1;
static int senzor1 = 1;

module_param(senzor0, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(senzor0, "An integer value of SENSOR0.");

module_param(senzor1, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(senzor1, "An integer value of SENSOR0.");
...
{
#ifdef TEST
    static char prepreka;

    prepreka = GetGpioPinValue(GPIO_12);
    if(prepreka == 0)
    	printk(KERN_INFO "gpio_driver: IR senzor ima prepreku ispred sebe.\n");
    else
	printk(KERN_INFO "gpio_driver: IR senzor nema prepreku ispred sebe.\n");
#endif
...
int gpio_driver_init(void)
{
    ...
    /*IR sensor*/
    SetGpioPinDirection(GPIO_16, GPIO_DIRECTION_OUT);	//VCC
    SetGpioPinDirection(GPIO_26, GPIO_DIRECTION_OUT);	//VCC

    if(senzor0 == 0)                                    // Gasimo tranzistore ako je poslata 0 prilikom inicijalizacije. Gasimo PNP tranzistor slanje 1 na bazu odnosno setovanjem pina
	SetGpioPin(GPIO_16);
    if(senzor1 == 0)
	SetGpioPin(GPIO_26);
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos)
{
    ...
    gpio_driver_buffer[data_size++] = '0' + GetGpioPinValue(GPIO_12);
    gpio_driver_buffer[data_size++] = '0' + GetGpioPinValue(GPIO_23);
    gpio_driver_buffer[data_size++] = '0' + GetGpioPinValue(GPIO_19);
    gpio_driver_buffer[data_size++] = '0' + GetGpioPinValue(GPIO_24);
...
static ssize_t gpio_driver_write(struct file *filp, const char *buf, size_t len, loff_t *f_pos)
{
	...
	printk(KERN_INFO "gpio_driver_buffer : %s" , gpio_driver_buffer);
        ...
        senzor = strsep(&gpio_driver_buffer,  " ");
        num = strsep(&gpio_driver_buffer, " ");
        state = gpio_driver_buffer;

        printk(KERN_INFO "%s: num=%s, state=%s", senzor, num, state);           // SENZOR 0 0 - nulti senzor na 0 (gasimo ga)
