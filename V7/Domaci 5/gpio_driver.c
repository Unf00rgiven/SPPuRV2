...
static enum hrtimer_restart stopwatch_timer_callback(struct hrtimer *param)
{
    ...
    if (brojac % 2 != 0) 
    { SetGpioPin(GPIO_06); }
    else { ClearGpioPin(GPIO_06);}

    // druga dioda
    if ((brojac > 1 && brojac < 4) || (brojac > 5 && brojac < 8) || (brojac > 9 && brojac < 12) || (brojac > 13))
    { SetGpioPin(GPIO_13); }
    else { ClearGpioPin(GPIO_13); }

    // treca dioda
    if ((brojac > 3 && brojac < 8) || (brojac > 11))
    { SetGpioPin(GPIO_19); }
    else { ClearGpioPin(GPIO_19); }

    // cetvrta  dioda
    if (brojac > 7)
    { SetGpioPin(GPIO_26); }
    else { ClearGpioPin(GPIO_26); }
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos)
{
	...
	gpio_driver_buffer[data_size++] = '0' + GetGpioPinValue(GPIO_16);
	gpio_driver_buffer[data_size++] = '0' + GetGpioPinValue(GPIO_20);
    gpio_driver_buffer[data_size++] = '0' + GetGpioPinValue(GPIO_21);
...
static ssize_t gpio_driver_write(struct file *filp, const char *buf, size_t len, loff_t *f_pos)
{
    ...
    else
    {
        ...
        led_chr = strsep(&gpio_driver_buffer,  " ");
        num = strsep(&gpio_driver_buffer, " ");
        state = gpio_driver_buffer;
        
        printk(KERN_INFO "%s: num=%s, state=%s", led_chr, num, state);
